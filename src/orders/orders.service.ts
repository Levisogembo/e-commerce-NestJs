...
export class OrdersService {
  ...
  async createOrder(userId: string, payload: createOrderInput) {
    ...
    try {
      ...
      const productValues = payload.items.map((item) => {
        const match = productChecks.find(
          (res) => res.product.productId === item.productId,
        );
  ...
  async handleMpesaCallback(data: handleMpesaCallbackDto) {
    this.logger.log(
      `Processing mpesa callback for requestId: ${data.checkoutRequestId}`,
    );
    ...
    const foundUser = await this.userRepository.findOne({
      where: { userId: foundOrder.user.userId },
    });
      ...
      if (foundOrder.couponId && foundOrder.discountAmount) {
        try {
          await this.couponService.redeemCoupon(
            foundOrder.couponId,
            foundOrder.user.userId,
            foundOrder.orderId,
            foundOrder.discountAmount,
          );
          this.logger.log(
            `Coupon ${foundOrder.couponId} redeemed for order ${foundOrder.orderId}`,
          );
        } catch (error) {
  ...
  async getAllOrders(page: number, limit: number, searchOptions?) {
    const { year, orderNumber, status, paidFrom, paidUntil, paymentMethod } =
      searchOptions || {};
    ...
    if (paymentMethod) {
      query.andWhere('order.paymentMethod ILIKE :paymentMethod', {
    ...
    const [orders, total] = await query
      ...
      .addSelect(['product.productId', 'product.name', 'product.price'])
      .skip((page - 1) * limit)
      .take(limit)
  ...
  async exportOrders(searchOptions) {
    const { year, orderNumber, status, paidFrom, paidUntil, paymentMethod } =
      searchOptions || {};
  ...
  async cancelOrder(orderId: string, userId: string) {
    ...
    if (!cancellableStatuses.includes(foundOrder.status))
      throw new ConflictException(
        'Only orders with status PENDING, PENDING_PAYMENT, or PROCESSING can be cancelled',
      );
    ...
    await this.orderRepository.manager.transaction(
      async (transactionManager) => {
        ...
        for (const item of savedOrders) {
          this.logger.log(
            `   Releasing product ${item.Product.productId}: ${item.quantity} units`,
          );
          await transactionManager
            .createQueryBuilder()
            .update(Product)
