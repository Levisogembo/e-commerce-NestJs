...
export class productService {
  ...
  async createNewProduct(
    { category, ...data }: createProductInput,
    fileMetadata: imageInput,
  ) {
    ...
    const newProduct = await this.productRepository.manager.transaction(
      async (transactionManager) => {
        ...
        await this.redisInventoryService.deleteByPattern(
          this.CATEGORY_PAGE_PATTERN,
        );
        return savedProduct;
      },
    );
  ...
  async getProducts(
    ...
    const { productName, categoryName } = searchOptions || {};
    let where: {name?,category?} = {};
    if (productName) {
  ...
  > {
    const [foundProducts, total] = await this.productRepository.findAndCount({
  ...
  async updateProduct(
    productId: string,
    { category, ...data }: updateProductDto,
    fileMetadata: updateImage,
  ) {
    const cacheKey = `product:${productId}`;
