...
export class OrderProcessor extends WorkerHost {
    ...
    private async handleOrderProcessing(job: Job) {
        const { orderId, userId, items, total, paymentMethod, phoneNumber } = job.data
        ...
        try {
            ...
            //initate mpesa stk push
            this.logger.log('Processing payment...')
            if (/Mpesa/i.test(paymentMethod)) {
                //handle mpesa payments
                const customerPhone = phoneNumber || user.address?.phoneNumber
                if (!customerPhone) throw new HttpException('Phone number required', HttpStatus.BAD_REQUEST)

                try {
                    const stkResponse = await this.mpesaService.initiateStkPush(customerPhone, total, orderId)
                    this.logger.log(`Mpesa STK push initiated successfully`)
                    this.logger.log(`CheckoutID: ${stkResponse.CheckoutRequestID}`)

                    //save checkoutrequest id for mpesa callback processing
                    await this.ordersRepository.update(orderId, {
                        mpesaCheckoutRequestId: stkResponse.CheckoutRequestID,
                        status: orderStatus.PENDING_PAYMENT
                    })

                    this.logger.log(`CheckoutId saved and order updated to pending payment`)
                    this.logger.log(`Payment prompt sent to ${customerPhone}`)
                    ...
                    return {success:true, orderId, checkoutRequestId: stkResponse.CheckoutRequestID}
                } catch (error) {
                    this.logger.error(`M-Pesa initiation failed: ${error.message}`)
                    await this.releaseInventory(orderId)
                    await this.ordersRepository.update(orderId, { status: orderStatus.PAYMENT_FAILED })
                    throw error
                }
    ...
    private async commitInventory(orderId: string): Promise<void> {
        ...
        await this.ordersRepository.manager.transaction(async (manager) => {
            ...
            const commitedOrderItems = await manager
                .createQueryBuilder(orderItems, 'orderItem')
                .leftJoinAndSelect('orderItem.product', 'product')
                .where('orderItem.orderId = :orderId', { orderId })
                .getMany();

            this.logger.log(`Found ${orderItems.length} items to commit`);

            for (const item of commitedOrderItems) {
        ...
        this.logger.log(`Inventory committed for order ${orderId}`);
    }
}